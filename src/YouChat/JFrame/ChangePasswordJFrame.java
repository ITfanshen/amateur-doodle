...
public class ChangePasswordJFrame extends JFrame implements MouseListener {
    ...
    public ChangePasswordJFrame(ArrayList<User> allUsers) {
        this.Users = allUsers;
        //初始化注册界面
        initFrame();
        initView();
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == submit) {
            ...
            if (username.getText().isEmpty()) {
                GetJDialog.showJDialog("用户名不能为空");
                return;
            }
            //4.判断用户名是否已经重复
            if (!containsUsername(username.getText())) {
                GetJDialog.showJDialog("用户名不存在，请先注册");
                return;
            }
            //3.判断用户名的格式是否正确
            ...
            if (!containsIdNumber(IDNumber.getText())) {
                ...
                GetJDialog.showJDialog("此身份证还未注册过账号，请先注册");
                return;
            }
            //1.密码不能为空
            if (String.valueOf(password.getPassword()).isEmpty() || String.valueOf(confirmPassword.getPassword()).isEmpty()) {
                GetJDialog.showJDialog("密码不能为空");
                return;
            }
            //3.判断密码的格式是否正确
            ...
            //修改信息
            try {
                changInfo(username.getText(), IDNumber.getText(), String.valueOf(confirmPassword.getPassword()));
            } catch (Exception ex) {
                throw new RuntimeException(ex);
            }
            //10.提示修改成功
            GetJDialog.showJDialog("修改密码成功");
            //关闭注册界面，打开登录界面
    ...
    private void initView() {
        ...
        passwordText.setBounds(100, 140, 95, 20);
        //添加密码输入框
        password.setBounds(175, 135, 200, 30);

        //添加再次输入密码的文本
        JLabel confirmPasswordText = new JLabel("确认新密码:");
        confirmPasswordText.setBounds(100, 200, 95, 20);
        //添加再次输入密码的输入框
        confirmPassword.setBounds(175, 195, 200, 30);

        //身份证号文本
        JLabel IdNumber = new JLabel("身份证号：");
        IdNumber.setBounds(100, 80, 70, 20);
        //身份证号输入框
        IDNumber.setBounds(175, 75, 200, 30);
    ...
    //HuTool
    private void changInfo(String userName, String idNumber, String newPassword) throws Exception {
        ArrayList<User> users = new ArrayList<>();
        for (User user : Users) {
            if (userName.equals(user.getUsername()) && idNumber.equals(user.getIdNumber())) {
                user.setPassword(newPassword);
            }
        }

        // 清空文件内容
        FileUtil.writeBytes(new byte[0], "E:\\Java\\YouChat\\src\\YouChat\\UserInfo\\userInformation");
        for (User user : Users) {
            users.add(new User(encrypt(user.getUsername()), encrypt(user.getPassword()), encrypt(user.getIdNumber())));
        }
